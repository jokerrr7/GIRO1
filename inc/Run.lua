...
giro = Token:match("(%d+)")
redis:set(giro..":VERSION",1)
redis:set(giro..":SUDO_ID:",SUDO_USER)
redis:set(giro..":DataCenter:",'German')
redis:set(giro..":UserNameBot:",BOT_User)
redis:set(giro..":NameBot:",BOT_NAME)
redis:hset(giro..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3giro_INSTALL","Yes")
info = {}
...
giro = Token:match("(%d+)")
our_id = tonumber(giro)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(giro..":UserNameBot:")
SUDO_ID = tonumber(redis:get(giro..":SUDO_ID:"))
SUDO_USER = redis:hgetall(giro..'username:'..SUDO_ID).username
version = redis:get(giro..":VERSION")
DataCenter = redis:get(giro..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..giro..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(giro..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(giro..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(giro..'group:add'..msg.chat_id_) then 
...
msg.Rank = 1
elseif redis:sismember(giro..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور '
msg.TheRank = 'مطور البوت '
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(giro..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي '
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(giro..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء '
msg.TheRank = 'المنشىء '
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(giro..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير ' 
msg.TheRank = 'مدير البوت ' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(giro..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن '
msg.TheRank = 'ادمن في البوت '
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(giro..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(giro..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(giro..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(giro..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(giro..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(giro..':WELCOME_BOT'),[[•مـرحبآ آنآ بوت آسـمـي ]]..redis:get(giro..':NameBot:')..[[•آختصـآصـي حمـآيه‌‏ آلمـجمـوعآت
...
if msg.GroupActive and redis:get(giro..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(giro..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(giro..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(giro..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.giro then 
if msg.text and ScriptFile.igiro then
for k, giro in pairs(ScriptFile.giro) do
local SearchText = msg.text:match(giro)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.igiro(msg,{msg.text:match(giro)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",giro.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dgiro then
if ScriptFile.dgiro(msg) == false then
return false
end
...
if Plug.giro then 
if msg.text and Plug.igiro then
for k, giro in pairs(Plug.giro) do
local SearchText = msg.text:match(giro)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(giro..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(giro..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.igiro(msg,{msg.text:match(giro)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",giro.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source giro")
end 

end
else
print("The File Script.lua Not Runing in The Source giro")
...
function tdcli_update_callback(data)
	...
	redis:srem(giro..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/jokerrr7/master/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/jokerrr7file/master/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/jokerrr7file/master/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/jokerrr7/master/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/jokerrr7/master/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/jokerrr7/master/plugins/games.lua','./plugins/games.lua')
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	download_file('https://raw.githubusercontent.com/jokerrr7/master/inc/Run.lua','./inc/Run.lua')
	download_file('https://raw.githubusercontent.com/jokerrr7/master/inc/Script.lua','./inc/Script.lua')
	download_file('https://raw.githubusercontent.com/jokerrr7/master/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/jokerrr7/master/inc/locks.lua','./inc/locks.lua')
	download_file('https://raw.githubusercontent.com/jokerrr7/master/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/jokerrr7/master/plugins/games.lua','./plugins/games.lua')
	...
	mmd = redis:get(giro..":SUDO_ID:")
redis:hset(giro..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(giro..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(giro..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(giro..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(giro..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(giro..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(giro..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(giro..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(giro..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(giro..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(giro..'group:add'..data.chat_.id_) then
	redis:set(giro..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(giro..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(giro..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(giro..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	giro..'group:name'..k,IDS.Title,
	giro..'num_msg_giro'..k,5,
	giro..'group:add'..k,true,
	giro..'lock_link'..k,true,
	giro..'lock_spam'..k,true,
	giro..'lock_webpage'..k,true,
	giro..'lock_markdown'..k,true,
	giro..'lock_flood'..k,true,
	giro..'lock_bots'..k,true,
	giro..'mute_forward'..k,true,
	giro..'mute_contact'..k,true,
	giro..'mute_document'..k,true,
	giro..'mute_inline'..k,true,
	giro..'lock_username'..k,true,
	giro..'replay'..k,true
	)
	redis:sadd(giro..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(giro..'username:'..ID,'username',user)
	redis:sadd(giro..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(giro..'username:'..ID,'username',user)
	redis:sadd(giro..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(giro..'username:'..ID,'username',user)
	redis:sadd(giro..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'•~** تم رفع آلنسـخه‏‏ آلآحتيآطـيه\n•~** حآليآ عدد مـجمـوعآتك هيه *'..redis:scard(giro..'group:ids')..'* ®️\n✓')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(giro..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(giro..'username:'..data.user_.id_).username
	...
	redis:hset(giro..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
